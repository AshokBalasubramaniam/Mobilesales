...
import { MESSAGE_TYPE, OFFER_STATUS, NOTIFICATION_TYPE, ROLES } from '../config/constants';
...
type ConversationWithParticipants = Populated<IConversation, { participants: PopulatedChatParticipant[]; mobile?: PopulatedChatMobile }>;
...
export const otherParticipant = (conversation: IConversation, userId: Types.ObjectId): Types.ObjectId | undefined =>
 ...
 * dispute moderation.
 */
const assertAccess = (conversation: IConversation, user: AuthenticatedUser): void => {
...
const pushMessageToConversation = async (conversation: IConversation, message: IMessage, senderId: Types.ObjectId): Promise<void> => {
...
export const startConversation = asyncHandler(async (req: Request<Record<string, never>, unknown, StartConversationBody>, res: Response) => {
...
export const listMyConversations = asyncHandler(async (req: Request<Record<string, never>, unknown, unknown, PaginationOnlyQuery>, res: Response) => {
  ...
  const withMeta = conversations.map((c) => {
    const obj = c.toObject() as unknown as ConversationWithParticipants & { unreadCount?: number; otherParticipant?: PopulatedChatParticipant };
...
export const getConversationById = asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
...
export const listAllConversationsAdmin = asyncHandler(async (req: Request<Record<string, never>, unknown, unknown, PaginationOnlyQuery>, res: Response) => {
  const { page, limit, skip } = getPagination(req.query);
...
export const getMessages = asyncHandler(async (req: Request<{ id: string }, unknown, unknown, MessageListQuery>, res: Response) => {
  const conversation = await Conversation.findById(req.params.id);
  if (!conversation) throw ApiError.notFound('Conversation not found');
  assertAccess(conversation, req.user!);
...
export const sendTextMessage = asyncHandler(async (req: Request<{ id: string }, unknown, SendTextMessageBody>, res: Response) => {
  const conversation = await Conversation.findById(req.params.id);
  if (!conversation) throw ApiError.notFound('Conversation not found');
  assertAccess(conversation, req.user!);
...
export const sendMediaMessage = asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
  const conversation = await Conversation.findById(req.params.id);
  if (!conversation) throw ApiError.notFound('Conversation not found');
  assertAccess(conversation, req.user!);
...
export const sendOffer = asyncHandler(async (req: Request<{ id: string }, unknown, SendOfferBody>, res: Response) => {
  const conversation = await Conversation.findById(req.params.id);
  if (!conversation) throw ApiError.notFound('Conversation not found');
  assertAccess(conversation, req.user!);
...
export const respondToOffer = asyncHandler(async (req: Request<{ id: string; messageId: string }, unknown, RespondToOfferBody>, res: Response) => {
  const conversation = await Conversation.findById(req.params.id);
  if (!conversation) throw ApiError.notFound('Conversation not found');
  assertAccess(conversation, req.user!);
...
export const sendLocation = asyncHandler(async (req: Request<{ id: string }, unknown, SendLocationBody>, res: Response) => {
  const conversation = await Conversation.findById(req.params.id);
  if (!conversation) throw ApiError.notFound('Conversation not found');
  assertAccess(conversation, req.user!);
...
export const logCallEvent = asyncHandler(async (req: Request<{ id: string }, unknown, LogCallEventBody>, res: Response) => {
  const conversation = await Conversation.findById(req.params.id);
  if (!conversation) throw ApiError.notFound('Conversation not found');
  assertAccess(conversation, req.user!);
...
export const blockConversation = asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
  const conversation = await Conversation.findById(req.params.id);
  if (!conversation) throw ApiError.notFound('Conversation not found');
  assertAccess(conversation, req.user!);
