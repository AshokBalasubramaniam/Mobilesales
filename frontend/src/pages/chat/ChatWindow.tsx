...
// The conversation participants / otherParticipant populated on Conversation
// (src/types/models.ts) only carry a narrow Pick<User, ...>; the real payload
// also includes sellerProfile/lastSeen (used for the verified badge and
// "last seen" text below) — extend locally rather than widening the shared type.
type ChatParticipant = Pick<User, '_id' | 'name' | 'avatar' | 'lastSeen' | 'sellerProfile'>;
...
const ChatWindow = () => {
  ...
  const typingUserIds = useAppSelector(selectTypingForConversation(conversationId));
  ...
  const participants = conversation?.participants as ChatParticipant[] | undefined;
  const otherUser = (participants?.find((p) => p._id !== user?._id) || conversation?.otherParticipant) as ChatParticipant | undefined;
  const isOtherOnline = Boolean(otherUser?._id && onlineUserIds.includes(otherUser._id));
  const otherLastSeen = (otherUser?._id && lastSeenByUserId[otherUser._id]) || otherUser?.lastSeen;
  const call = useVideoCall({ conversationId: conversationId ?? '', otherUserId: otherUser?._id });
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-4">
        {messages.map((message) => {
          const sender = typeof message.sender === 'string' ? undefined : message.sender;
          ...
          return (
            <MessageBubble
              ...
              isOwn={isOwn}
              senderLabel={
                isThirdParty ? `${sender?.name || 'Support'}${sender?.role === 'admin' ? ' (Mobile Sales Support)' : ''}` : undefined
              }
