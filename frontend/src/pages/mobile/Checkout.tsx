...
const Checkout = () => {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      toast.success(
        `Coupon applied! You saved ${formatCurrency(data.data.discount)}`,
      );
    } catch (err) {
      toast.error(
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!user.isEmailVerified)
      return toast.error("Please verify your email before placing an order");
    ...
    try {
      ...
      const { data: payRes } = await api.post<ApiResponse<RazorpayOrder>>(
        "/payments/orders",
        { orderId: order._id },
      );
      ...
      if (!window.Razorpay)
        return toast.error("Could not load payment gateway");
      ...
      <div className={classes.formCard}>
        ...
        <div className={classes.totalsBlock}>
          <div className={classes.summaryRow}>
            <span className={classes.summaryLabel}>Item price</span>
            <span>{formatCurrency(mobile.price)}</span>
          </div>
          <div className={classes.summaryRow}>
            <span className={classes.summaryLabel}>Delivery charge</span>
