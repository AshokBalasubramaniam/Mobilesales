...
const Disputes = () => {
  ...
  const handleResolve = async (status: 'resolved' | 'rejected') => {
    ...
    try {
      ...
      toast.error((isAxiosError<{ message?: string }>(err) && err.response?.data?.message) || 'Could not update dispute');
    }
  };
    ...
    <div>
      ...
      {disputes.length === 0 ? (
        ...
        <div className="space-y-3">
          {disputes.map((d) => {
            // Backend populates the full order document; the shared Order Pick type only
            // declares _id/orderNumber, so widen locally to also read pricing.
            const order = typeof d.order === 'string' ? null : (d.order as Pick<Order, '_id' | 'orderNumber'> & { pricing?: OrderPricing });
            ...
            return (
              <div key={d._id} className="rounded-xl border border-gray-200 p-4 dark:border-gray-800">
                <div className="flex items-start justify-between">
                  <div>
                    <Link to={PATHS.orderDetail(order?._id)} className="font-medium hover:text-brand-600">
                      Order #{order?.orderNumber} — {formatCurrency(order?.pricing?.totalAmount)}
                    </Link>
