...
const Users = () => {
  ...
  const [filters, setFilters] = useState({ role: '', isBlocked: '', q: '' });
    ...
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <Input
          ...
          className="max-w-xs"
        />
        <Select value={filters.role} onChange={(e) => setFilters({ ...filters, role: e.target.value })} className="w-36">
          <option value="">All roles</option>
          <option value="buyer">Buyer</option>
          <option value="seller">Seller</option>
          <option value="admin">Admin</option>
        </Select>
        <Select value={filters.isBlocked} onChange={(e) => setFilters({ ...filters, isBlocked: e.target.value })} className="w-36">
                  ...
                  <td className="p-3">
                    {u.role === 'seller' && (
                      <div className="flex items-center gap-2">
                        <Badge variant={u.sellerProfile?.verificationStatus === 'approved' ? 'green' : u.sellerProfile?.verificationStatus === 'pending' ? 'amber' : 'gray'}>
