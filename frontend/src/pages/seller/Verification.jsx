...
const Verification = () => {
  const { user } = useAuth();
  const [files, setFiles] = useState({ aadhaar: null, pan: null, selfie: null, purchaseBill: null });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!files.aadhaar || !files.pan || !files.selfie) {
      return toast.error('Aadhaar, PAN, and selfie are required');
    }
    setSubmitting(true);
    try {
      ...
      toast.error(err.response?.data?.message || 'Could not submit documents');
    } finally {
      setSubmitting(false);
    }
  };
