...
const SalesBarChart = ({ data, labelKey = '_id' }) => {
  ...
  return (
    <div className="flex items-end gap-2 overflow-x-auto pb-2" style={{ height: 180 }}>
      {data.map((d) => (
        <div key={d[labelKey]} className="group relative flex min-w-8 flex-1 flex-col items-center justify-end gap-1">
          ...
          />
          <span className="text-[10px] text-gray-400">{d[labelKey].slice(5)}</span>
        </div>
      ))}
    </div>
  );
