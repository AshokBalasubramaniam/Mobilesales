...
const classes = {
  ...
  remotePlaceholder: 'flex size-full flex-col items-center justify-center gap-3',
  localVideo:
    'absolute right-3 bottom-3 h-28 w-20 rounded-lg object-cover ring-2 ring-white/50',
  endCallButton: 'mt-6 flex items-center gap-2 rounded-full bg-red-600 px-6 py-3',
  endCallIcon: 'size-5',
  callButton: 'rounded-full p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800',
      ...
      {status === 'ringing' && !remoteStream ? (
        <div className={classes.ringingContainer}>
          ...
          <div className={classes.ringingActions}>
            <button onClick={() => incomingOffer && answerCall(incomingOffer)} className={classes.answerButton}>
        ...
        <>
          <div className={classes.videoContainer}>
            {remoteStream ? (
              <video ref={remoteRef} autoPlay playsInline className={classes.remoteVideo} />
            ) : (
            ...
            <video ref={localRef} autoPlay playsInline muted className={classes.localVideo} />
          </div>
          <button onClick={() => endCall(true)} className={classes.endCallButton}>
