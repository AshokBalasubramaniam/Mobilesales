...
type CheckboxOption = string | number | { value: string | number; label: string };
...
const FilterSidebar = ({ filters, onChange, onClear }: FilterSidebarProps) => {
  ...
  const set = (patch: Partial<MobileFilters>) => onChange({ ...filters, ...patch });

  const toggleArrayFilter = (key: string, values: string[]) => set({ [key]: values });
