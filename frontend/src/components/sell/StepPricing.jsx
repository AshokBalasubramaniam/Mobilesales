...
const StepPricing = ({ form, setForm }) => {
      ...
      <div>
        ...
        {suggestion && (
          <div className="mt-2 rounded-lg bg-brand-50 p-3 text-sm dark:bg-brand-900/20">
            ...
            <button type="button" onClick={() => setForm({ ...form, price: suggestion.suggestedPrice })} className="ml-2 font-medium text-brand-700 hover:underline dark:text-brand-300">
              Use this price
            </button>
          </div>
        )}
        ...
        {form.purchaseBill ? (
          ...
          <label className="flex w-fit cursor-pointer items-center gap-2 rounded-lg border border-dashed border-gray-300 px-4 py-2 text-sm text-gray-500 dark:border-gray-700">
            ...
            <input
              ...
              hidden
              onChange={(e) => e.target.files[0] && setForm({ ...form, purchaseBill: e.target.files[0] })}
            />
          </label>
        )}
      </div>
